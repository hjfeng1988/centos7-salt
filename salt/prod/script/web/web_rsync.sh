...
# rsync nginx配置
ssh -o StrictHostKeyChecking=no -i $rsa $ruser@$rip "mkdir -p $remote_bak_dir/web"
rsync $opts -e "ssh -o StrictHostKeyChecking=no -i $rsa" --log-file=$rsync_log $web_bak_dir/nginx_conf_* $ruser@$rip:$remote_bak_dir/web
